...
class GameMap {
    ...
    randomizeGround(tile, x, y) {
        ...
        if (Math.random() * 100 <= 10) tile = Config.Tiles.Weed;else if (Math.random() * 100 <= 30) tile = Config.Tiles.Rock;
        return tile;
    }
    ...
    generateWater(force) {
        ...
        while (gen == false) {
            ...
            for (let i = 0; i < width; i++) {
                for (let u = 0; u < height; u++) {
                    if (this.grid[x + i] != null && this.grid[x + i][y + u] == Config.Tiles.Ground) {
                        if (this.grid[x][y - 2] == Config.Tiles.Lava || this.grid[x][y - 3] == Config.Tiles.Lava || this.grid[x][y + 2] == Config.Tiles.Lava || this.grid[x][y + 3] == Config.Tiles.Lava || this.grid[x - 2] != null && this.grid[x - 2][y] == Config.Tiles.Lava || this.grid[x - 3] != null && this.grid[x - 3][y] == Config.Tiles.Lava || this.grid[x + 2] != null && this.grid[x + 2][y] == Config.Tiles.Lava || this.grid[x + 3] != null && this.grid[x + 3][y] == Config.Tiles.Lava || this.grid[x - 2] != null && this.grid[x - 2][y - 2] == Config.Tiles.Lava || this.grid[x - 3] != null && this.grid[x - 3][y - 3] == Config.Tiles.Lava || this.grid[x + 2] != null && this.grid[x + 2][y - 2] == Config.Tiles.Lava || this.grid[x + 3] != null && this.grid[x + 3][y - 3] == Config.Tiles.Lava || this.grid[x - 2] != null && this.grid[x - 2][y + 2] == Config.Tiles.Lava || this.grid[x - 3] != null && this.grid[x - 3][y + 3] == Config.Tiles.Lava || this.grid[x + 2] != null && this.grid[x + 2][y + 2] == Config.Tiles.Lava || this.grid[x + 3] != null && this.grid[x + 3][y + 3] == Config.Tiles.Lava) continue;
...
class Program {
    ...
    load() {
        PIXI.loader.add("assets/animations/Hero.json").add("assets/animations/Badguy.json").add("assets/animations/Pig.json").add("assets/animations/Tileset.json").add("assets/images/GUI/StatUI_background.png").add("assets/images/GUI/Heart.png").add("assets/animations/Particles.json").add("assets/images/Elements/Hole.png").load(() => {
            this.setup();
        });
    }
    setup() {
...
class ControllerTouch {
    constructor(player, bounds) {
        ...
        document.getElementById("touch").addEventListener("touchstart", e => {
            for (let i = 0; i < e.changedTouches.length; i++) this.start(e.changedTouches[i]);
        }, true);
        document.getElementById("touch").addEventListener("touchmove", e => {
            for (let i = 0; i < e.changedTouches.length; i++) this.move(e.changedTouches[i]);
        }, true);
    ...
    move(e) {
        if (this.touch == null || e.identifier != this.touch.identifier) return;
        let coords = {
        ...
        if (Math.abs(coords.x) > Math.abs(coords.y)) {
            if (coords.x < 0) this.left();else if (coords.x > 0) this.right();
        } else if (Math.abs(coords.x) < Math.abs(coords.y)) {
            if (coords.y < 0) this.up();else if (coords.y > 0) this.down();
        ...
        console.log(e);
        if (e != null && this.touch != null && this.touch.identifier != e.identifier) return;
        this.touch = null;
...
class EntityHole {
    ...
    setPlayer(player) {
        ...
        let y = 64;
        if (this.sprite.y > Program.GetInstance().App().renderer.height / 2) y = Program.GetInstance().App().renderer.height - 32 * 3;
...
class EntityPlayer extends EntityWalking {
    ...
    updateEmitter() {
        if (this.emitter != null) {
            ...
            if (this.onFire > 0) this.emitter.config.life = this.onFire;
        }
    }
    update(delta) {
        super.update(delta);
...
class ParticleEmitter {
    ...
    destroy(execute = true) {
        if (this.callback != null && execute) this.callback();
        this.scene.unregisterParticleEmitter(this.id);
        this.container.destroy({ children: true });
    }
    update(delta) {
        if (this.life != Infinity) this.life -= delta;
        ...
        this.entities.forEach(entity => {
            ...
            if (entity.solid) {
                this.entities.forEach(other => {
                    ...
                    if (normal != null) {
                        other.hit(entity);
                        if (other instanceof EntityPig || other.solid == false) return;
